...
const AdminDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const [shopsRes, statsRes] = await Promise.all([
        axios.get('http://localhost:5000/api/admin/shops', { headers }),
  ...
  const handleShopStatusUpdate = async (shopId: number, status: string) => {
    try {
      const token = localStorage.getItem('adminToken');
      await axios.put(`http://localhost:5000/api/admin/shops/${shopId}/status`, 
        { status },
  ...
  const handleSubscriptionUpdate = async (shopId: number, plan: string, monthlyFee: number) => {
    try {
      const token = localStorage.getItem('adminToken');
      await axios.put(`http://localhost:5000/api/admin/shops/${shopId}/subscription`, 
